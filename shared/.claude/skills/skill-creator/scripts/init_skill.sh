...
init_skill() {
    ...
    local skill_dir="$(cd "$path" 2>/dev/null && pwd)/$skill_name" || skill_dir="$(pwd)/$path/$skill_name"
    ...
    cat > "$skill_dir/SKILL.md" << 'SKILL_TEMPLATE_END'
---
...
SKILL_TEMPLATE_END
    ...
    sed -i '' "s/SKILL_TITLE_PLACEHOLDER/$skill_title/g" "$skill_dir/SKILL.md"
    echo "✅ Created SKILL.md"

    # Create scripts directory with example script
    mkdir -p "$skill_dir/scripts"
    cat > "$skill_dir/scripts/example.sh" << 'EXAMPLE_SCRIPT_END'
#!/bin/bash

# Example helper script for SKILL_NAME_PLACEHOLDER
#
...
echo "This is an example script for SKILL_NAME_PLACEHOLDER"
# TODO: Add actual script logic here
# This could be data processing, file conversion, API calls, etc.
EXAMPLE_SCRIPT_END

    sed -i '' "s/SKILL_NAME_PLACEHOLDER/$skill_name/g" "$skill_dir/scripts/example.sh"
    chmod 755 "$skill_dir/scripts/example.sh"
    echo "✅ Created scripts/example.sh"

    # Create references directory with example reference doc
    mkdir -p "$skill_dir/references"
    cat > "$skill_dir/references/api_reference.md" << 'EXAMPLE_REFERENCE_END'
...
- Troubleshooting
- Best practices
EXAMPLE_REFERENCE_END

    sed -i '' "s/SKILL_TITLE_PLACEHOLDER/$skill_title/g" "$skill_dir/references/api_reference.md"
    echo "✅ Created references/api_reference.md"

    # Create assets directory with example asset placeholder
    mkdir -p "$skill_dir/assets"
    cat > "$skill_dir/assets/example_asset.txt" << 'EXAMPLE_ASSET_END'
# Example Asset File
...
main() {
    if [ $# -lt 3 ] || [ "$2" != "--path" ]; then
        echo "Usage: init_skill.sh <skill-name> --path <path>"
        echo ""
        ...
        echo ""
        echo "Examples:"
        echo "  init_skill.sh my-new-skill --path skills/public"
        echo "  init_skill.sh my-api-helper --path skills/private"
        echo "  init_skill.sh custom-skill --path /custom/location"
        exit 1
    fi
